...
Displays total monetary value across the filtered customer base. Despite the "AVG Score" label in the dashboard, this reflects cumulative revenue contribution — the name refers to the monetary dimension of the RFM score, not a statistical average.
...
| Revenue over time | Bar chart | `SUM(total_revenue)` by `year_month` | Monthly granularity, Oct 2016–Aug 2018 |
...
| Orders by year | Donut chart | `SUM(total_orders)` by year | 2016 ≈ 1%, 2017 ≈ 45%, 2018 ≈ 54% |
...
| Customer Segment pivot | Pivot table | Rows: `customer_segment` · Cols: `f_score` · Values: `Record Count` | Shows distribution of frequency score within each segment |
| Customers by segment | Horizontal bar | `COUNT(customer_id)` by `customer_segment` | Champions and Potential Loyalists each ≈ 25K customers |
| Segment share | Donut chart | `COUNT(customer_id)` by `customer_segment` | Champions 25.2% · Potential Loyalists 25% · Loyal Customers 24.8% · At Risk 18.5% · Lost 6.5% |
...
|---|---|---|
| Champions | ≥ 10 | Bought recently, buy often, spend the most |
| Loyal Customers | ≥ 8 | Regular buyers with high spend |
| Potential Loyalists | ≥ 6 | Recent buyers with moderate frequency |
| At Risk | ≥ 4 | Haven't bought recently but were valuable |
...
| Visual | Type | Fields | Notes |
|---|---|---|---|
| Retention by Cohort | Table with heatmap | `cohort_month` · `retained_customers` · `cohort_customers` | Heat map coloring: blue = retained volume, orange = cohort size. Shows 23 monthly cohorts from Sep 2016 to Jun 2018 |
| Period filter | Date range control | `cohort_month` | Narrows visible cohort window |

**Cohort logic** (from SQL): each row represents a cohort defined by the customer's first purchase month. `retained_customers` counts how many from that cohort placed at least one additional order. Because the Olist dataset ends in August 2018, later cohorts (2018) show retention close to 100% — they haven't had enough time to churn yet. This is the same boundary-effect observed in the churn analysis of the SaaS project.
